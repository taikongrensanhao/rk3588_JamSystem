...
def _load_model():
    ...
    """
    global _model, _device, _analyzer
...
def _find_psd_peaks(iq_data, fs, topk=6, threshold_db=8, nperseg=4096):
    """
    在频谱中寻找明显的窄带峰值。
...
def signal_sync_recovery(iq_data):
    ...
    h_rrc = get_rrc(0.35, SPS, 12)
    iq_matched = signal.filtfilt(h_rrc, 1.0, iq_norm)
    ...
    best_fine = 0.0
    best_cluster = -1.0
    for candidate in np.linspace(-1000, 1000, 101):
...
def _restore_single_tone(iq_data):
    ...
    4. 用候选 EVM 打分，保留最优结果。
    """
    peak_candidates = _find_psd_peaks(iq_data, FS, topk=3, threshold_db=5, nperseg=4096)
    ...
    for peak_freq in peak_candidates:
        for bandwidth in (6e3, 8e3, 10e3, 14e3):
            cleaned = _apply_notches(iq_data, [peak_freq], bw_hz=bandwidth, fs=FS, order=4)
            for cutoff in (420e3, 500e3, 620e3):
                sos_lp = signal.butter(6, cutoff, btype="low", fs=FS, output="sos")
                candidate = signal.sosfiltfilt(sos_lp, cleaned).astype(np.complex64)
...
def _restore_white_noise(iq_data):
    ...
    sos_lp = signal.butter(8, 650e3, btype="low", fs=FS, output="sos")
...
def restore_signal(iq_data, label):
    ...
    if label in {"narrowband", "comb"}:
        ...
        sos_lp = signal.butter(6, 500e3, btype="low", fs=FS, output="sos")
    ...
    if label in {"wideband_barrage", "noise_fm"}:
        ...
        sos_lp = signal.butter(8, 700e3, btype="low", fs=FS, output="sos")
        clean = signal.sosfiltfilt(sos_lp, clean)
        sos_bp = signal.butter(4, [10e3, 800e3], btype="bandpass", fs=FS, output="sos")
        clean = signal.sosfiltfilt(sos_bp, clean)
...
def _run_restoration(iq_processed, label):
    """
    执行单帧的还原与指标评估。
    ...
    try:
        raw_const = signal_sync_recovery(iq_processed)
    except Exception:
        raw_const = iq_processed[::SPS]
    ...
    if method == "bypass":
        ...
        try:
            clean_const = signal_sync_recovery(clean_wave)
            clean_const, _ = refine_constellation(clean_const)
        except Exception:
            clean_const = clean_wave[::SPS]
...
def predict_loop(file_path):
    ...
    while True:
        ...
        try:
            restore_info = _run_restoration(iq_processed, label)
